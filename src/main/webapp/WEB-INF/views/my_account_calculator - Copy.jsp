...
/* function setReceivingCountry(crCode,countryName,crName){ */
	function setReceivingCountry(tocountrydata){
	 ...
	 $('#reciveClick').empty();
	 var i='<span><img src="resources/images/countryimages/'+tocountrydata[2].toLowerCase()+'.svg" width="30px" height="30px"" class="rounded-circle"><span>'+tocountrydata[2]+'</span><i class="fa fa-chevron-down"></i></span>';
	...
	$('.ReciveInSendFrom').append('<img src="resources/images/countryimages/'+tocountrydata[2].toLowerCase()+'.svg" width="30px" height="30px"">'+tocountrydata[3]+'<span class="float-right">'+tocountrydata[11]+' '+tocountrydata[2]+'</span>');
	...
	success : function(response){
		...
		//alert('sc===='+sc);
		 ...
		 $.each($.parseJSON(branchList),function(idx,obj){
				 if(sc===obj.fromCountry.currencyCode){
					 $('#clickk').empty();
...
//alert('fromcountry  = '+fromcountry.split("-")[1]);
	   var opt='';
		var display='backend';
	   ...
	   $.ajax({
		 ...
		 type : "get",
		 data : "branchId="+fromcountry.split("-")[1]+"&Amount="+sendingAmount+"&display="+display,
		 async : false,
		 success : function(response){
	...
	//alert('response = ='+response);
 		...
 		$('#mops_fee_list').val(mopList);
 			//alert('mopList = '+mopList);
				 ...
				 $.each($.parseJSON(mopList),function(idx,obj){
					var hlght_mop='';
					if(mp!=''){
						//alert('mp is not empty---');
						 if(mp==obj.modeOfPaymentId.modeOfPayment){
							//alert('if condition of mop');	  
							 $('#paywith').empty();
				 ...
				 if(hlght_mop==obj.modeOfPaymentId.modeOfPayment){
					 ...
					 opt=opt+'<button type="button" id="'+obj.modeOfPaymentId.modeOfPayment.replace(/\s/g,'_')+'" onclick=getModeOfPaymentFee("'+obj.modeOfPaymentId.modeOfPayment.replace(/\s/g,"&nbsp;")+'","'+obj.fee+'","'+obj.rateType+'"); class="dropdown-item"><li><i class="fa fa-bank"></i> '+obj.modeOfPaymentId.modeOfPayment+'</li></button>';
				 }
		 ...
		 error : function(error,xhr,status){
			 alert('getModeOfPaymentError1244 = '+error+xhr+status);
		 }
			...
			$.each($.parseJSON(branchList),function(idx,obj){
				if(rc!=''){
					//alert('rc is notempty');
					 ...
					 }else if(rc==obj.fromCountry.currencyCode){
						 $('#reciveClick').empty();
						 $('#reciveClick').append('<span><img src="/images/countryimages/'+obj.fromCountry.currencyCode.toLowerCase()+'.svg" width="30px" height="30px"" class="rounded-circle"><span>&nbsp;&nbsp;'+obj.fromCountry.currencyCode+'</span><i class="fa fa-chevron-down"></i></span>');
						 $('#toCountry').val(obj.fromCountry.countryISO+'-'+obj.branchId+'-'+obj.fromCountry.currencyCode+'-'+obj.fromCountry.countryName+'-'+obj.minimumTransaction+'-'+obj.maximumTransaction);
						 $('#receivein').empty();
...
<script type="text/javascript">
				 ...
				 for(var i=0;i<modeOfPmt.length;i++){
					 ...
					 //opt=opt+'<option value="'+obj.modeOfPayment+','+obj.fee+'">'+obj.modeOfPayment.toUpperCase()+'</option>';
						 ...
						 if(i==0){
								   ...
								   $('#modeOfPayment').val(obj.modeOfPayment+','+obj.fee+','+obj.rateType);
								   hlght_mop=obj.modeOfPayment.replace(/\s/g,"&nbsp;");
								   $('#cnf_payment').empty();
									$('#cnf_payment').append(hlght_mop);
						 }
						 if(hlght_mop==obj.modeOfPayment){
							 opt=opt+'<button type="button" id="'+obj.modeOfPayment+'" onclick=getModeOfPaymentFee("'+obj.modeOfPayment.replace(/\s/g,"&nbsp;")+'","'+obj.fee+'","'+obj.rateType+'"); class="dropdown-item_high"><li><i class="fa fa-bank"></i> '+obj.modeOfPayment+'</li></button>';
						 }else{
							 opt=opt+'<button type="button" id="'+obj.modeOfPayment.replace(/\s/g,'_')+'" onclick=getModeOfPaymentFee("'+obj.modeOfPayment.replace(/\s/g,"&nbsp;")+'","'+obj.fee+'","'+obj.rateType+'"); class="dropdown-item"><li><i class="fa fa-bank"></i> '+obj.modeOfPayment+'</li></button>';
						 }
					 }
					 });
	...
	function validate(){
 		...
 		//getCurrencysendingAmount=$('#sendingAmount').val();
...
// 		} 
	...
	if(type=='receivingCountry'){
		...
		var options='';
		$.ajax({
			url : "getAllSendingBranchesExceptGivenBranch",
			type : "GET",
			data : data,
			async : false,
			success : function(response){
...
//			alert('receivingBranch = '+receivingBranch);
		...
		//alert('pt==='+pt);
				$.each($.parseJSON(paymentType),function(idx,obj){
					...
					if(pt!=''){
						//alert('pt is not empty==');
						if(ptype==''){
							   if(pt==obj.paymentTypes.paymentType){
								  // alert('if payment type iton 00');
							  //deliveryTime=obj.deliveryTime;
							   ...
							   $('#paymentType').val(ptype);
								//$('#deliveryTime').val(ptype);
	 
						   }
						   

					}else{
					//alert('ptype is empty');
						if(ptype==''){
						   if(i==0){
							  // alert('iton 00');
						  //deliveryTime=obj.deliveryTime;
...
function getCurrencyRates(){
	...
	//alert('data is  = '+"fromBranchId="+sendingBranchId.split('-')[1]+"&toBranchId="+receivingBranchId.split('-')[1]);
	...
	$.ajax({
		...
		$.ajax({
			...
			success : function(response){
				...
				//alert('service_fee_list==='+sr_fee_list);
			},
			error  : function(error,xhr,status){
		...
		doCalculation('send');
		  //alert('receivingBranch = '+receivingBranchId);
...
function doCalculation(type){
	...
	//alert('modeOfPayment123==='+modeOfPayment);
...
var mopsList=$('#mops_fee_list').val();
	//alert('service_fee_list==='+mopsList);
		if(type=='send'){
			//alert('inside send');
...
function makeItHighlighted(selected_item,current_id){
 ...
 $('#cnf_payment').empty();
	...
	$('#modeOfPayment').val(mop+','+fee+','+rateType+','+mop);
...
<input type="hidden" name="rc" id="rc" value="${rc}">
...
<!-- 						<input type="hidden" id="deliveryTime" name=deliveryTime>				
 -->                <div class="row brd-t-1">
                    ...
                    <div class="col-md-8 col-sm-7 col-xs-7 fromNum">
                       ...
                       <c:if test="${sendingAmount ne null }">
                         <input type="text" name="sendingAmount" id="sendingAmount" onkeypress="return onlyNumber(event);" onkeyup="doCalculation('send')" value="${sendingAmount}">
                       </c:if>
                        <c:if test="${sendingAmount eq null }">
                ...
                <div class="row mnlp">
                    <div class="col-md-8 col-sm-7 col-xs-7 fromNum">
                        ...
                        <c:if test="${receivingAmount ne null }">
                         <input type="text" name="receivingAmount" id="receivingAmount"  onkeypress="return onlyNumber(event);" onkeyup="doCalculation('receive')" value="${receivingAmount}" style="position: relative;">
                        </c:if>
                        <c:if test="${receivingAmount eq null }">
