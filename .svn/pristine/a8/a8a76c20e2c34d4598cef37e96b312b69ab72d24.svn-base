...
<select class="form-control" name="userType" id="userType">
						...
						<option value="ROLE_BRANCH">Agents</option>
						<option value="ROLE_CUSTOMER">Remitter Users</option>
					
</select>
</div>
<div class="col-md-3">
<label>	Login Start Date</label>
    ...
    <tbody class="list">
     <c:forEach items="${list}" var="list">
      <tr>
         ...
         <td class="text-center">${list.logOnTime}</td>
         <td class="text-center">${list.logOffTime}</td>
         <td class="text-center">${list.acessResult}</td>
...
function getList(){
	...
	//alert('logondate===='+logondate);
	//alert('logoffdate==='+logoffdate);
	//alert('userType===='+userType);
	//alert('username==='+username);
	var data='';
	var url='';
	if(logondate!='' && username=='' && userType==''){
		if(logoffdate==''){
			alert('please select logoff date ');
			return false;
		}
		data="logondate="+logondate+"&logoffdate="+logoffdate;
		url="accessloglistbwdates";
	}
	if(userType!='' && logondate!='' && logoffdate!='' && username==''){
		data="logondate="+logondate+"&logoffdate="+logoffdate+"&userType="+userType;
		url="accessloglistBwDatesAndUsertype";
		
	}
	if(userType=='' && logondate!='' && logoffdate!='' && username!=''){
		data="logondate="+logondate+"&logoffdate="+logoffdate+"&username="+username;
		url="accessloglistBwDatesAndUsername";
		
	}

	if(userType!='' && logondate!='' && logoffdate!='' && username!=''){
		data="logondate="+logondate+"&logoffdate="+logoffdate+"&userType="+userType+"&username="+username;
		url="accessloglistBwDatesAndUsertypeAndUserName";
	
	}
	if(userType!='' && logondate=='' && logoffdate=='' && username=='' ){
	data="userType="+userType;
	url="getlistbyuserType";
	}
	if(userType=='' && logondate=='' && logoffdate=='' && username!='' ){
	...
	if(userType!='' && logondate=='' && logoffdate=='' && username!='' ){
		...
		url="accessloglistbyUsertypeAndUserName";
		}
			  ...
			  $.each($.parseJSON(txnList),function(idx,obj){
				  var count='0';
					var td='';
					 ...
					 rows=rows+"<tr style='background:#fff'><td class='text-center'>"+obj.userName+"</td><td class='text-center'>"+obj.userType+"</td><td class='text-center'> "+obj.ipAddress+"</td><td class='text-center'> "+obj.logOnTime+"</td><td class='text-center'> "+obj.logOffTime+"</td><td class='text-center'> "+obj.acessResult+"</td></tr>";
