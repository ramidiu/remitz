...
function checkBankName()
{
	...
	var opt='';
	 $.ajax({
			url : "bankNameCheck",
			type : "POST",
			data :"accountNumber="+accountNumber+"&destinationHouse="+destinationHouse+"&branchId="+branchId,
			async: false,
			success:function(response){
				//alert('response==='+response);
				if(response!='no'){
				if(response == 'SUCCESSFUL'){ 
						...
						$('#fail1').show();
						$('#fail1').empty();
						$('#fail1').append("Invalid Account Details.....");
						$('#fail').show();
						$('#fail').empty();
						$('#fail').append("Invalid Account Details.....");
 ...
 */		  /* receivingAmt=Number(dollerAmt)*Number(fromCurrencyRates); */
		  ...
		  var data="fromBranchId="+bid+"&toBranchId="+toBranchId+"&amount="+entered_amount+"&paymentType="+p;
			...
			$('#admin_fee').val(Number(fees)-(Number(adfee)+Number(afee)));
			$('#rec_fee').val(Number(afee));
			var enterAmount=$('#enterAmount').val();
			if(enterAmount=='SD')
			{
...
<div class="form-row">
   <label id="emsg"></label>
								...
								<option value="Female">Female</option>
              					...
              					<div class="form-group col-md-4 ">
              						<label for="inputEmail4">Town</label>
              						<input type="text" class="form-control" name="custCityState1" id="cityState1"  value="${tx.customer.cityOrState}" placeholder="Enter City or State Name *" >
              					</div>
										</div>
										<div class="form-row">
              					<div class="form-group col-md-4">
              			...
              			<div class="form-row">
                             ...
                             <div  id="utilityTrDiv">
              					...
              					<input type="text" name="invoiceNumber" id="invoiceNumber" class="form-control">			
               					...
               					<div class="form-group col-md-4" id="cp">
                					<label for="inputEmail4">Bank To Pay</label>
                       				<select class="form-control" name="bankToPay" id="bankToPay" onchange="checkBankName()">
         	<option value="">--select--</option>
              			...
              			<div  id="cashTrDiv">
              					...
              					<div class="form-group col-md-4" id="btr3">
                					<label for="inputEmail4">collectionPointBank</label>
            <select class="form-control" name="collectionPointBank" id="collectionPointBank" >
         	<option value="">--select--</option>
