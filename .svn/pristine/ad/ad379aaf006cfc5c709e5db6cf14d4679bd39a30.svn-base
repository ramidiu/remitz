...
function readData(statustype){
	 ...
	 $.each($("input[name='customerId']:checked"), function(){
		 ...
		 $.ajax({
			...
			type : "POST",
			data :"customerId="+checkboxval+"&customerStatus="+statustype,
			async: false,
				  ...
				  $.each($.parseJSON(customerList),function(idx,obj){
					...
					/*   var allMtos=obj.mtos.split(",");
					  var mtosList='';
						...
						tb=tb+'<tbody id="tbody"><tr><td><input type="checkbox" name="customerId" value="'+obj.customerId+'" ></td><td>'+obj.customerId+'</td><td><a href="${pageContext.request.contextPath}/adminCustomerprofile?custRegId='+obj.customerId+'" target="_blank" style="color:blue">'+obj.firstName+'</td><td>'+obj.mobileNumber+'</td><td>'+firstIdProofName+'</td><td><img src="/images/customerimages/'+obj.customerId+'/'+obj.firstIdProofDoc+'" width="100" height="55" alt="Please Upload Primary ID Proof"/></td><td>'+obj.status+'</td><td><a href="admincustomereditprofile?customerId='+obj.customerId+'" class="btn btn-primary btn-alt-primary">edit</a></td></tr>';
				 }); // now we don't have resouces folder 
				
				 $('#responseDiv').append(tb+'</tbody></table>');
			}else{
				$('#responseDiv').empty();
				$('#responseDiv').append('<table class="table table-bordered table-hover "><thead><tr class="table-primary"> <th style="width:5%;"></th><th style="width:9%;">CUSTOMER ID</th><th style="width:15%;">CUSTOMER NAME</th><th style="width:12%;">CONTACT NUMBER</th><th style="width:11%;">ID PROOF TYPE</th><th style="width:4%;">PRIMARY ID PROOF</th> <th style="width:5%">STATUS</th><th style="width:5%">EDIT</th></tr></thead><tr><td colspan="11" align="center" style="color:red">No Records  Avilable.</td></tr></tbody></table>');
			}
		},
		error : function(xhr,status,error){
...
<div class="abc pull-right">
                         <button type="button" id="activebtn" onclick="readData('active');">Approve</button>&nbsp;&nbsp;<button type="button" id="deactivebtn" onclick="readData('deactive');">DeActive</button>&nbsp;&nbsp;<button id="suspendbtn"  onclick="readData('suspend');">Suspend</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Filter By Status
                         <select  name="customerStatus" id="customerStatus" onchange="onchangeCustomerStatus();"  class="mah" style="color: #0168fa;background-color: white;border: 1px solid #0168fa !important;padding: 6px 13px 6px 10px;border-radius: 5px;position: relative;left: 4px;">
                               <option value="active" selected>Active</option><option value="pending">Pending</option><option value="deactive">DeActive</option> <option value="suspend">Suspend</option></select>
